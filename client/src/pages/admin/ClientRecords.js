    ...
    return (
        <Layout style={{ margin: 0, padding: 0 }}>
            <div style={{ backgroundColor: '#E8F4FE', minHeight: '100vh', padding: '1px' }}>
                ...
                {/* Search Input */}
                <input
                    className='ml-10 pt-2 pb-2 p-2 py-1 pr-10 w-25'
                    type="text"
                    placeholder="Search by Name of Pet"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                />
                <button className='button-link ml-3' onClick={() => setSearchQuery('')}>Clear</button>

                <Link className='button-link mb-2 text-white ml-3' to="/admin/create-client-records">
                    Add Client Record
                </Link>
