...
const viewRecordDetailsController = async (req, res) => {
  ...
  const viewFullDetailsController = async(req, res) => {
    ...
    const handleDoctorAppointmentsController = async (req, res) => {
        try {
            const appointments = await appointmentModel.find(); // Fetch all doctor appointments
            res.status(200).send({
                success: true,
...
const updateAppointmentStatusController = async (req, res) => {
    try {
        ...
        // Additional logic, such as sending notifications, can be added here.
        res.status(200).send({
            success: true,
            message: `Appointment ${status}d`,
        });
    } catch (error) {
        console.log(error);
        res.status(500).send({
            success: false,
            error,
            message: `Error in updating the appointment status`,
        });
    }
};
...
module.exports = { 
    ...
    handleDoctorAppointmentsController,
